...
export default function Dashboard({ userLang, user }: { userLang: string, user: any }) {
        const [userName, setUserName] = useState('');
        const [news, setNews] = useState<newsItemType[]>([]);
        const [traslatedNews, setTranslatedNews] = useState<any[]>([]);
        ...
        useEffect(() => {
                (async () => {
                        await axios.get(`https://oursos-backend-production.up.railway.app/news`)
                                .then((res) => {
                                        setNews(res.data);
                                }).then(async (res) => {
                                        let newsData: any = {}
                                        news.map((data: newsItemType, i) => {
                                                newsData[i] = { "snippet": data.snippet, "title": data.title }
                                        })
                                        await axios.post("https://oursos-backend-production.up.railway.app/translateobject", { "translateObject": newsData, "lang": userLang })
                                                .then(res => {
                                                        setTranslatedNews(res.data);
                                                })
                                })
                                .catch((err) => {
                                        console.log(err);
                ...
                <View style={tw.style(`flex`, `h-full`, `flex-col`, `justify-between`, `w-full`, `relative`)}>
                        <ScrollView style={tw.style(`px-4`)}>
                                ...
                                <Searchbar
                                        ...
                                        value={searchQuery}
                                />
                                <Slider onToggleSnackBar={onToggleSnackBar} data={news} translatedData={traslatedNews} />
                                <MapComp height={300} />
                                <View style={tw.style(`h-[300px]`, `w-full`, `py-4`)}>
                                        <View style={tw.style(`flex`, `flex-row`, `justify-between`, `w-full`, `items-center`)}>
                                                ...
                                                <View style={tw.style(`mb-[60px]`)}>

                                                        {
                                                                pins?.map((pin: any, i: number) => {
                                                                        let text = "Vancouver"
                                                                        let imgUrl = "https://www.planetware.com/wpimages/2020/03/canada-best-cities-vancouver-british-columbia.jpg"
                                                                        return (
                                                                                <ImageText key={i} text={text} style="rounded" imageUrl={imgUrl} />
                                                                        )
                                        ...
                                        <ScrollView horizontal
                                                ...
                                                contentContainerStyle={{ justifyContent: 'flex-start' }} >
                                                {
                                                        friends?.map((pin: any, i: number) => {
                                                                ...
                                                                return (
                                                                        <ImageText key={i} text={text} style="circle" imageUrl={imgUrl} />
                                                                )
                                                        })
                                                }
                                                {/* </View> */}
